...
- Resolve: (a) cmdAuthResume right after appending the auth_resume marker resolves all open auth for the bee, by "auth-resume" (daemon-down safe); (b) reconcile observes auth bounded by auth_resume with request still open → resolve by "auth-resume". Claude HSR may also recover automatically when the bound home's on-disk OAuth chain is well-formed, unexpired, refreshable, and identity-compatible. The daemon persists one attempt per runtime generation, applies exponential backoff, and hard-stops after two uninterrupted attempts.
- Cancel: generation exit / new incarnation / retire → scope-closed.

Auth prompt durability: normal local-HSR turns stay in `pending-turns/` until a non-auth `turn_end`. A login-required error leaves the exact text journaled. Recovery stages that text in `auth-replay.json` across stop/revive, appends `auth_resume` before replay, then hands it back to the new host's pending-turn journal. Legacy pre-journal failures may use `lastPrompt` only when its timestamp is provably inside the failed structured turn; otherwise a durable manual-action request tells the operator to resend instead of silently losing text.

Prevention decision: Honeybee does not restart Claude merely because `.credentials.json` changed. The long-lived child has no supported token-reload channel, so reacting to file mtime would still require a disruptive same-session restart and could interrupt healthy active turns whenever a sibling rotates the shared chain. Recovery therefore keys off the reproducible structured login failure, where restart is already necessary; prompt journaling makes that reactive cure lossless.
...
## Writer wiring
Daemon: registry stage `requestReconciles` / `reconcileRequests` runs before `authRecoveries` / `recoverAuthNeeded`, then `dispatchNeedsInput`. The reconciler implementation is src/daemon/requestSweep.ts, createRequestReconciler() — stateful across ticks, built in wiring.ts next to the two dispatchers. Boot tick: listBeesWithRequests() readdir + read seeds cache bee → {openIds, generation}. Steady state: touch a bee only when (a) snapshot shows pending needs_input or unbounded auth, (b) cache says open records exist, (c) record is kill_failed, (d) runtimeGeneration moved past a cached open record; else zero-IO skip. Cache advisory only — every mutation re-reads under lock. Runs only when sessionsSnapshotTrusted; per-bee skip for hsrUnavailable; never throws (outcome array, needsInput.ts style); log request.open/resolve/cancel rows via registry log().
