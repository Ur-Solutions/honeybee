...
test("Kimi remote HSR is rejected before transport or credential handling", async () => {
  assert.match(remoteHarnessPolicyError("kimi") ?? "", /local-only/);
  ...
  try {
    await assert.rejects(
      spawnBee({ agent: "kimi", node, extraArgs: [], cwd: root, yolo: true }),
      /kimi HSR is local-only: remote credential delivery is not implemented or tested/,
    );
  } finally {
