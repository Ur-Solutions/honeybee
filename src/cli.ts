  ...
  const groups: Array<{ title: string; rows: Array<[string, string, string]> }> = [
    ...
    },
    {
      ...
      rows: [
        ...
        ["buz", "<send|inbox|read|…>", "addressed messaging: four-tier delivery + per-bee policy"],
    ...
    },
    {
      title: "Observe",
      rows: [
        ["list", "", "show active sessions with state (alias: ps; --history for full catalog)"],
        ["state", "<ls|explain>", "BeeView read model: display state, requests, results, freshness (--json)"],
        ["bees", "", "grouped fuzzy fleet TUI, or one-shot machine output with --json"],
    ...
    },
    {
      ...
      rows: [
        ...
        ["auth-resume", "<bee>", "reuse valid disk credentials (or require login), resume, and replay the failed prompt"],
